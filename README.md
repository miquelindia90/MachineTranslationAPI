...
4. Once the API server is running, you can make translation requests using curl or any other HTTP client. Here's an example curl request:
   ...
   ```bash
   curl -X POST -H "Content-Type: application/json" -d '{"language_pair": "en-nb", "text": "Get quick answers from Wingate by Wyndham Oklahoma City Airport staff and past guests."}' http://localhost:<HOST_PORT>/translate
   ```
