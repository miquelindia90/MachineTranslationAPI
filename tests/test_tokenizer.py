...
def test_split_alphanumeric():
    ...
    assert (
        split_alphanumeric("tvillingarna 'den vackra'.")
        == "tvillingarna ' den vackra ' ."
    )
...
def test_MTTokenizer_training_model():
    ...
    tokenizer.save_tokens_dictionary(
        "tests/examples/output_source_tokens.json",
        "tests/examples/output_target_tokens.json",
    )
    with open("tests/examples/reference_source_tokens.json", "r") as target_file, open(
        "tests/examples/output_source_tokens.json", "r"
    ) as output_file:
        target_dict = json.load(target_file)
        output_dict = json.load(output_file)
        assert target_dict == output_dict
    os.remove("tests/examples/output_source_tokens.json")
    with open("tests/examples/reference_target_tokens.json", "r") as target_file, open(
        "tests/examples/output_target_tokens.json", "r"
    ) as output_file:
        target_dict = json.load(target_file)
        output_dict = json.load(output_file)
        assert target_dict == output_dict
...
def test_MTTokenizer_inference_model():
    ...
    assert tokenizer.source_lang_sentence_to_id_list("And it is also the case") == [
        1,
        2,
        3,
        4,
    ...
    assert tokenizer.target_lang_sentence_to_id_list("Så är det även i dag med") == [
        1,
        2,
        3,
        4,
